...
### 2026-06-19 — Make the Settings → Providers Refresh (↻) button hybrid: registry ∪ live `/v1/models`, merge by `id` (ATO-210)
- **Context:** `handleRefreshModels` in [`web-app/src/routes/settings/providers/$providerName.tsx`](web-app/src/routes/settings/providers/$providerName.tsx) only called `useProviderRegistryStore.refresh()` + `getProviders()` (local engines + registry). This produced a false *"No new models found"* toast for: (a) custom providers not in `atomic-chat-conf` (custom vLLM/llama.cpp, etc.) — the registry has no entries for them so the result was always empty; (b) external registry providers (NVIDIA NIM, etc.) whose live deployment exposes models not listed in the registry manifest. The toast also never distinguished between "registry genuinely has nothing new" and "the live fetch errored" — both showed the same misleading success message ([ATO-210](https://linear.app/atomicchat/issue/ATO-210)).
- **Decision (minimal, web-app only, no registry or Rust change):** After refreshing the registry and retrieving `fresh` providers, also call the **existing** `serviceHub.providers().fetchModelsFromProvider(provider)` (line 144 in `tauri.ts`) in a `try/catch` block. Merge results: `union(registry models, live IDs)` deduped by `id`, with registry metadata (name, capabilities) taking priority for overlapping IDs; IDs that only appear on the live server are appended as bare `{ id }` Model objects. Three distinct toast outcomes: (1) new models found → success with count; (2) live fetch failed (no base_url, network error, auth error) → `toast.warning` with `refreshModelsLiveFailed` showing the error; (3) registry + server genuinely empty → existing `noNewModels` success. New locale key `refreshModelsLiveFailed` added to EN + RU (other locales fall back to EN).
- **Consequences:** Custom providers and external providers with live `/v1/models` endpoints now surface their models on Refresh. Cloud providers that fail the live fetch (no base_url, auth failure, network error) fall back to registry-only behavior and show an accurate warning instead of a misleading "no new models". The `fetchModelsFromProvider` function itself throws when `base_url` is missing, which is caught and treated as a non-fatal live-fetch error. **Deliberately NOT done (out of scope):** the P1 treatment for distinguishing live-fetch error sub-types (network vs auth vs not-found) is tracked separately as mentioned in ATO-210; the current warning toast surfaces the raw error message from `fetchModelsFromProvider` which already provides actionable detail. **Verified:** code review of the three-way conditional; `Model` type usage consistent with existing usage in the same file (global `.d.ts` declaration). Dependencies: none new; `yarn lint` / `tsc -b` infra not available in the sandbox (no installed node_modules), consistent with prior bug-fix ADRs that relied on rolldown/build as the authoritative compile check.
- **Owner:** team.
- **Links:** [ATO-210](https://linear.app/atomicchat/issue/ATO-210), files: [`web-app/src/routes/settings/providers/$providerName.tsx`](web-app/src/routes/settings/providers/$providerName.tsx) (`handleRefreshModels`), [`web-app/src/locales/en/providers.json`](web-app/src/locales/en/providers.json), [`web-app/src/locales/ru/providers.json`](web-app/src/locales/ru/providers.json).
