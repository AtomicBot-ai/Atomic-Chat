...
- **Context:** Tauri treats every Cargo `[[bin]]` target as an application
  binary to bundle. The feature-gated `gaia-eval` target was skipped by the
  normal release build, but the macOS bundler still tried to copy it and
  failed because `target/universal-apple-darwin/release/gaia-eval` did not
  exist.
- **Decision:** Disable Cargo's automatic binary discovery, declare the desktop
  ...
  `gaia-eval` as a feature-gated `[[example]]` target run through
  `cargo run --example gaia-eval`. Keep the evaluator's source, feature gate,
