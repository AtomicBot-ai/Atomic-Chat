...
describe('useModelProvider migrations', () => {
  ...
  describe('v15 — cloud providers must be connected to stay active', () => {
    const runV15 = (providers: unknown[]) => {
      const persistApi = (useModelProvider as any).persist
      const migrate = persistApi?.getOptions().migrate as
        | ((state: unknown, version: number) => any)
        | undefined
    ...
    it('deactivates cloud providers that were never given an API key', () => {
      const migrated = runV15([
        {
          provider: 'openai',
          active: true,
          api_key: '',
      ...
      expect(migrated.providers[0].active).toBe(false)
    })

    it('keeps cloud providers that already carry a key', () => {
      const migrated = runV15([
        {
          provider: 'anthropic',
          active: true,
      ...
      expect(migrated.providers[0].active).toBe(true)
    })

    it('keeps keyless loopback providers such as Ollama', () => {
      const migrated = runV15([
        {
          provider: 'ollama',
          active: true,
    ...
    it('never touches local engines', () => {
      const migrated = runV15([
        {
      ...
      expect(migrated.providers[0].active).toBe(true)
      expect(migrated.providers[1].active).toBe(true)
    })
  })
})
