---
name: uupm-design
description: "Comprehensive design skill: brand identity, design tokens, UI styling, logo generation (55 styles), corporate identity programmes (CIP mockups), HTML presentations (Chart.js), banner design (22 styles, for social, ads, web and print), icon design (15 styles, SVG) and social photos for Facebook, X, LinkedIn, YouTube, Instagram, Pinterest and TikTok. Use to design a logo, create a CIP, generate mockups, build slides, design a banner, generate an icon or make social media images."
version: 2.1.0
requires_tools:
  - os.fs.read
  - os.fs.write
...
## Running this skill's scripts in Radium

Command lines below are written as `python scripts/<file>` or
`node scripts/<file>`. In Radium run them with `skill.run_script` instead:
put the path after `scripts/` in `script`, and every argument in `args`.

```json
{ "tool": "skill.run_script", "args": { "skill": "uupm-design",
  "script": "cip/core.py", "args": ["<first argument>", "--flag", "<value>"] } }
```

The script runs with the skill's own folder as its working directory.
Radium sets no `CLAUDE_PLUGIN_ROOT`, so never build a path from it.
Ask the user questions in your reply: Radium has no `AskUserQuestion` tool.
...
|------|-----------|---------|
| Brand identity, voice, assets | `uupm-brand` | Bundled sibling skill |
| Tokens, specs, CSS vars | `uupm-design-system` | Bundled sibling skill |
| shadcn/ui, Tailwind, code | `uupm-ui-styling` | Bundled sibling skill |
...
After generation, **ALWAYS** ask user about HTML preview via `AskUserQuestion`. If yes, use the bundled `uupm-ui-ux-pro-max` skill for the gallery.
...
2. **Research** — Read `references/banner-sizes-and-styles.md` and use the bundled `uupm-ui-ux-pro-max` skill for style and palette guidance; if browser research is available and permitted, collect 3–5 references
...
Multi-platform social image design: HTML/CSS → screenshot export. Uses the bundled `uupm-ui-ux-pro-max`, `uupm-brand`, and `uupm-design-system` skills; screenshot export runs through Chrome headless, Playwright, or Puppeteer (see the reference).
...
4. **Design** — bundled `uupm-brand` → `uupm-design-system` → `uupm-ui-ux-pro-max` skills; HTML per idea × size
