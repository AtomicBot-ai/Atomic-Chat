...
fn test_resolve_path() {
    ...
    // file:// path resolves relative to jan data folder
    let resolved_file = resolve_path(app.handle().clone(), "file://models/model.gguf");
    assert_eq!(
        resolved_file,
        data_folder.join(format!("models{}model.gguf", std::path::MAIN_SEPARATOR))
    );

    // http URL remains unchanged
    let resolved_http = resolve_path(app.handle().clone(), "https://example.com/model.bin");
    assert_eq!(
        resolved_http,
        std::path::PathBuf::from("https://example.com/model.bin")
    );
