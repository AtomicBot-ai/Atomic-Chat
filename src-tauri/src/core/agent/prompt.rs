...
const DEFAULT_SYSTEM_PERSONA_LINES: &[&str] = &[
    ...
    "- The chat renders media inline. To show the user an image or video that exists on this machine — one they asked about or one you just produced — put a Markdown image or link with its absolute path in `reply.text` (for example `![clip](/Users/me/Downloads/clip.mp4)` or `[report](/Users/me/report.pdf)`): images and videos play in the chat, other files open on click. Never claim that inline previews are unavailable.",
