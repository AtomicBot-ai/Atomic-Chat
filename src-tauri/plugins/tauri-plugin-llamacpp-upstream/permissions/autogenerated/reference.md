## Default Permission

Default permissions for the llamacpp plugin

#### This default permission set includes the following:

- `allow-cleanup-llama-processes`
- `allow-load-llama-model`
- `allow-cancel-llama-model-load`
- `allow-unload-llama-model`
- `allow-get-devices`
- `allow-get-runtime-device`
- `allow-check-spec-type-support`
- `allow-generate-api-key`
- `allow-is-process-running`
- `allow-get-random-port`
- `allow-find-session-by-model`
- `allow-get-loaded-models`
- `allow-get-all-sessions`
- `allow-get-session-by-model`
- `allow-read-gguf-metadata`
- `allow-estimate-kv-cache-size`
- `allow-get-model-size`
- `allow-is-model-supported`
- `allow-plan-model-load`
- `allow-map-old-backend-to-new`
- `allow-get-local-installed-backends`
- `allow-list-supported-backends`
- `allow-determine-supported-backends`
- `allow-get-supported-features`
- `allow-is-cuda-installed`
- `allow-find-latest-version-for-backend`
- `allow-prioritize-backends`
- `allow-parse-backend-version`
- `allow-check-backend-for-updates`
- `allow-remove-old-backend-versions`
- `allow-validate-backend-string`
- `allow-should-migrate-backend`
- `allow-handle-setting-update`
- `allow-install-bundled-backend`
- `allow-verify-backend-binary`
- `allow-fetch-manifest-http1`
- `allow-available-disk-space`

## Permission Table

<table>
<tr>
<th>Identifier</th>
<th>Description</th>
</tr>


<tr>
<td>

`llamacpp-upstream:allow-available-disk-space`

</td>
<td>

Enables the available_disk_space command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-available-disk-space`

</td>
<td>

Denies the available_disk_space command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-cancel-llama-model-load`

</td>
<td>

Enables the cancel_llama_model_load command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-cancel-llama-model-load`

</td>
<td>

Denies the cancel_llama_model_load command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-check-backend-for-updates`

</td>
<td>

Enables the check_backend_for_updates command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-check-backend-for-updates`

</td>
<td>

Denies the check_backend_for_updates command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-check-spec-type-support`

</td>
<td>

Enables the check_spec_type_support command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-check-spec-type-support`

</td>
<td>

Denies the check_spec_type_support command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-cleanup-llama-processes`

</td>
<td>

Enables the cleanup_llama_processes command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-cleanup-llama-processes`

</td>
<td>

Denies the cleanup_llama_processes command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-determine-supported-backends`

</td>
<td>

Enables the determine_supported_backends command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-determine-supported-backends`

</td>
<td>

Denies the determine_supported_backends command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-estimate-kv-cache-size`

</td>
<td>

Enables the estimate_kv_cache_size command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-estimate-kv-cache-size`

</td>
<td>

Denies the estimate_kv_cache_size command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-fetch-manifest-http1`

</td>
<td>

Enables the fetch_manifest_http1 command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-fetch-manifest-http1`

</td>
<td>

Denies the fetch_manifest_http1 command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-find-latest-version-for-backend`

</td>
<td>

Enables the find_latest_version_for_backend command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-find-latest-version-for-backend`

</td>
<td>

Denies the find_latest_version_for_backend command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-find-session-by-model`

</td>
<td>

Enables the find_session_by_model command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-find-session-by-model`

</td>
<td>

Denies the find_session_by_model command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-generate-api-key`

</td>
<td>

Enables the generate_api_key command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-generate-api-key`

</td>
<td>

Denies the generate_api_key command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-get-all-sessions`

</td>
<td>

Enables the get_all_sessions command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-get-all-sessions`

</td>
<td>

Denies the get_all_sessions command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-get-devices`

</td>
<td>

Enables the get_devices command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-get-devices`

</td>
<td>

Denies the get_devices command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-get-loaded-models`

</td>
<td>

Enables the get_loaded_models command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-get-loaded-models`

</td>
<td>

Denies the get_loaded_models command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-get-local-installed-backends`

</td>
<td>

Enables the get_local_installed_backends command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-get-local-installed-backends`

</td>
<td>

Denies the get_local_installed_backends command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-get-model-size`

</td>
<td>

Enables the get_model_size command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-get-model-size`

</td>
<td>

Denies the get_model_size command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-get-random-port`

</td>
<td>

Enables the get_random_port command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-get-random-port`

</td>
<td>

Denies the get_random_port command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-get-runtime-device`

</td>
<td>

Enables the get_runtime_device command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-get-runtime-device`

</td>
<td>

Denies the get_runtime_device command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-get-session-by-model`

</td>
<td>

Enables the get_session_by_model command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-get-session-by-model`

</td>
<td>

Denies the get_session_by_model command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-get-supported-features`

</td>
<td>

Enables the get_supported_features command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-get-supported-features`

</td>
<td>

Denies the get_supported_features command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-handle-setting-update`

</td>
<td>

Enables the handle_setting_update command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-handle-setting-update`

</td>
<td>

Denies the handle_setting_update command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-install-bundled-backend`

</td>
<td>

Enables the install_bundled_backend command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-install-bundled-backend`

</td>
<td>

Denies the install_bundled_backend command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-is-cuda-installed`

</td>
<td>

Enables the is_cuda_installed command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-is-cuda-installed`

</td>
<td>

Denies the is_cuda_installed command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-is-model-supported`

</td>
<td>

Enables the is_model_supported command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-is-model-supported`

</td>
<td>

Denies the is_model_supported command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-is-process-running`

</td>
<td>

Enables the is_process_running command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-is-process-running`

</td>
<td>

Denies the is_process_running command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-list-supported-backends`

</td>
<td>

Enables the list_supported_backends command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-list-supported-backends`

</td>
<td>

Denies the list_supported_backends command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-load-llama-model`

</td>
<td>

Enables the load_llama_model command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-load-llama-model`

</td>
<td>

Denies the load_llama_model command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-map-old-backend-to-new`

</td>
<td>

Enables the map_old_backend_to_new command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-map-old-backend-to-new`

</td>
<td>

Denies the map_old_backend_to_new command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-parse-backend-version`

</td>
<td>

Enables the parse_backend_version command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-parse-backend-version`

</td>
<td>

Denies the parse_backend_version command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-plan-model-load`

</td>
<td>

Enables the plan_model_load command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-plan-model-load`

</td>
<td>

Denies the plan_model_load command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-prioritize-backends`

</td>
<td>

Enables the prioritize_backends command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-prioritize-backends`

</td>
<td>

Denies the prioritize_backends command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-read-gguf-metadata`

</td>
<td>

Enables the read_gguf_metadata command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-read-gguf-metadata`

</td>
<td>

Denies the read_gguf_metadata command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-remove-old-backend-versions`

</td>
<td>

Enables the remove_old_backend_versions command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-remove-old-backend-versions`

</td>
<td>

Denies the remove_old_backend_versions command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-should-migrate-backend`

</td>
<td>

Enables the should_migrate_backend command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-should-migrate-backend`

</td>
<td>

Denies the should_migrate_backend command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-unload-llama-model`

</td>
<td>

Enables the unload_llama_model command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-unload-llama-model`

</td>
<td>

Denies the unload_llama_model command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-validate-backend-string`

</td>
<td>

Enables the validate_backend_string command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-validate-backend-string`

</td>
<td>

Denies the validate_backend_string command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:allow-verify-backend-binary`

</td>
<td>

Enables the verify_backend_binary command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`llamacpp-upstream:deny-verify-backend-binary`

</td>
<td>

Denies the verify_backend_binary command without any pre-configured scope.

</td>
</tr>
</table>
