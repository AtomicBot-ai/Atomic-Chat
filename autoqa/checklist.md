...
- [ ] Add 2 custom assistants to Radium 
...
- [ ] [0.6.9] Take a `gguf` file and delete the `.gguf` extensions from the file name, import it into Radium and verify that it works.
...
- [ ] [0.6.9] Make sure that Ollama set-up  as a custom provider work with Radium
In Custom Providers:
...
- [ ] [0.6.9] Make sure that you can send an image to a Local API Server and it also works (can set up Local API Server as a Custom Provider in Radium to test)
...
- [ ] Clicking on a listing will open up the model card information within Radium and render the HTML properly
...
## E. Assistants
- [ ] There is always at least one default Assistant which is Radium
- [ ] The default Radium assistant has `stream = True` by default 
...
- [ ] Click on `Reset` button in `Other` to factory reset the app:
	- [ ] All threads deleted
	- [ ] All Assistant deleted except for default Radium Assistant
...
- [ ] Clean up by deleting all the left over folder created by Radium
	- [ ] On MacOS
...
- [ ] Ensure that the fresh install of Radium launch
